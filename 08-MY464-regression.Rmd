...
\begin{equation}
r = \frac{s_{xy}}{s_{x}s_{y}} = \frac{\sum\left(X_{i}-\bar{X}\right)\left(Y_{i}-\bar{Y}\right)}{\sqrt{\sum\left(X_{i}-\bar{X}\right)^{2} \sum\left(Y_{i}-\bar{Y}\right)^{2}}}
...
(of the $\chi^{2}$ test, see first footnote in Chapter 