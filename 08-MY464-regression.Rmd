...
(\#eq:sdyx)
\end{equation} where the subscripts $x$ and $y$ identify the two
...
(\#eq:sxy)
\end{equation} This is a measure of linear association between $X$ and
...
(\#eq:corr)
\end{equation} This is the (sample) **correlation** coefficient, or
...
combined as \begin{equation}
Y_{i}=\alpha+\beta X_{i} +\epsilon_{i}
(\#eq:slinmodel)
\end{equation} where each $\epsilon_{i}$ is normally distributed
...
(\#eq:ols-a)
\end{equation} where $\bar{Y}$, $\bar{X}$, $s_{x}$ and $s_{xy}$ are the
...
residual standard deviation $\sigma$, is \begin{equation}
\hat{\sigma}=
\sqrt{
...
(\#eq:sigma-linreg)
\end{equation} where $k$ is here set equal to 1. This bears an
...
(\hat{Y}_{i}-\bar{Y})^{2}.
(\#eq:ss-decomp)
\end{equation} Here $SSM=\sum (\hat{Y}_{i}-\bar{Y})^{2}=TSS-SSE$ is
...
\sum (Y_{i}-\bar{Y})^{2}}.
(\#eq:R2)
\end{equation} This is the *proportion* of the total variation of $Y$ in
...
discussed here is that its value is zero, i.e. \begin{equation}
H_{0}:\; \beta=0.
(\#eq:H0betasimple)
\end{equation} Recall that when $\beta$ is 0, there is no linear association between the explanatory variable $X$ and the response
...
(\#eq:sebeta)
\end{equation} where $\hat{\sigma}$ is the estimated residual standard
...
(\#eq:tbeta)
\end{equation} The logic of this is the same as in previous
...
(\#eq:cibeta)
\end{equation} where the multiplier $t_{\alpha/2}^{(n-2)}$ is obtained
...
also be expressed in the concise form \begin{equation}
Y_{i} = \alpha
+\beta_{1}X_{1i}+\beta_{2}X_{2i}+\dots+\beta_{k}X_{ki}+\epsilon_{i}
(\#eq:mlinmodel)
\end{equation} where the error term (population residual)
...
-   **Estimates of the regression coefficients** are here denoted with
    ...
    \hat{\beta}_{k}X_{ki}.
    (\#eq:Yhat)
    \end{equation} The estimated regression coefficients are again
...
(\#eq:mu1)
\end{equation} for any values of $X_{1}$, $X_{2}$ and $X_{3}$. Suppose
...
(\#eq:mu2)
\end{equation} Subtracting (\@ref(eq:mu1)) from (\@ref(eq:mu2)) leaves us with
...
(\#eq:tbeta-m)
\end{equation} This is identical in form to statistic (\@ref(eq:tbeta))
...
are unrestricted}
(\#eq:H0beta-m)
\end{equation} against the alternative hypothesis
...
(\#eq:cibeta-m)
\end{equation} This is identical in form and interpretation to the
