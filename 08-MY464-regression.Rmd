...
(\#eq:ols-a)
\end{equation}
where $\bar{Y}$, $\bar{X}$, $s_{x}$ and $s_{xy}$ are the
...
\hat{\sigma}=\sqrt{\frac{\sum (Y_{i}-\hat{Y}_{i})^{2}}{n-(k+1)}}=\sqrt{\frac{SSE}{n-(k+1)}}
(\#eq:sigma-linreg)
\end{equation}
where $k$ is here set equal to 1. This bears an
...
(\#eq:R2)
\end{equation}
This is the *proportion* of the total variation of $Y$ in
...
(\#eq:H0betasimple)
\end{equation}
Recall that when $\beta$ is 0, there is no linear association between the explanatory variable $X$ and the response
...
(\#eq:sebeta)
\end{equation}
where $\hat{\sigma}$ is the estimated residual standard
...
(\#eq:cibeta)
\end{equation}
where the multiplier $t_{\alpha/2}^{(n-2)}$ is obtained
...
(\#eq:mu1)
\end{equation}
for any values of $X_{1}$, $X_{2}$ and $X_{3}$. Suppose
...
(\#eq:tbeta-m)
\end{equation}
This is identical in form to statistic (\@ref(eq:tbeta))
